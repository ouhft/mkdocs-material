(() => {
  ...
  var Observable = function() {
    function Observable36(subscribe) {
      if (subscribe) {
        this._subscribe = subscribe;
      }
    }
    Observable36.prototype.lift = function(operator) {
      var observable2 = new Observable36();
      observable2.source = this;
      observable2.operator = operator;
      return observable2;
    };
    Observable36.prototype.subscribe = function(observerOrNext, error, complete) {
      var subscriber = isSubscriber(observerOrNext) ? observerOrNext : new SafeSubscriber(observerOrNext, error, complete);
      if (config.useDeprecatedSynchronousErrorHandling) {
        this._deprecatedSyncErrorSubscribe(subscriber);
      } else {
        var _a2 = this, operator = _a2.operator, source = _a2.source;
        subscriber.add(operator ? operator.call(subscriber, source) : source ? this._subscribe(subscriber) : this._trySubscribe(subscriber));
      }
      return subscriber;
    };
    Observable36.prototype._deprecatedSyncErrorSubscribe = function(subscriber) {
    ...
    Observable36.prototype._trySubscribe = function(sink) {
      try {
    ...
    Observable36.prototype.forEach = function(next, promiseCtor) {
      var _this = this;
    ...
    Observable36.prototype._subscribe = function(subscriber) {
      var _a2;
      return (_a2 = this.source) === null || _a2 === void 0 ? void 0 : _a2.subscribe(subscriber);
    };
    Observable36.prototype[observable] = function() {
      return this;
    };
    Observable36.prototype.pipe = function() {
      var operations = [];
      for (var _i = 0; _i < arguments.length; _i++) {
    ...
    Observable36.prototype.toPromise = function(promiseCtor) {
      var _this = this;
      promiseCtor = getPromiseCtor(promiseCtor);
      return new promiseCtor(function(resolve, reject) {
        var value;
    ...
    Observable36.create = function(subscribe) {
      return new Observable36(subscribe);
    };
    return Observable36;
  }();
  ...
  function caughtSchedule(subscriber, scheduler, execute, delay2) {
    if (delay2 === void 0) {
      delay2 = 0;
    }
    ...
    return new Observable(function(subscriber) {
      ...
      subscriber.add(scheduler.schedule(function() {
        ...
        caughtSchedule(subscriber, scheduler, function() {
          var _a2 = iterator2.next(), value = _a2.value, done = _a2.done;
          if (done) {
            subscriber.complete();
          } else {
            subscriber.next(value);
  ...
  function share(options) {
    ...
    var _a2 = options.connector, connector = _a2 === void 0 ? function() {
      return new Subject();
    } : _a2, _b = options.resetOnComplete, resetOnComplete = _b === void 0 ? true : _b, _c = options.resetOnError, resetOnError = _c === void 0 ? true : _c, _d = options.resetOnRefCountZero, resetOnRefCountZero = _d === void 0 ? true : _d;
    var connection = null;
    var subject = null;
    var refCount = 0;
  ...
  function watchLocationHash() {
    return fromEvent(window, "hashchange").pipe(map(getLocationHash), startWith(getLocationHash()), filter((hash) => hash.length > 0), share());
  }
  function watchLocationTarget() {
    return watchLocationHash().pipe(switchMap((id) => of(getElement(`[id="${id}"]`))));
  }
  ...
  function watchPrint() {
    return merge(watchMedia("print").pipe(filter(Boolean)), fromEvent(window, "beforeprint")).pipe(mapTo(void 0));
  }
  ...
  config2.base = new URL(config2.base, getLocation()).toString().replace(/\/$/, "");
  ...
  function renderVersionSelector(versions) {
    ...
    const active = versions.find(({version, aliases}) => version === current || aliases.includes(current)) || versions[0];
    return /* @__PURE__ */ h("div", {
      class: "md-version"
    }, /* @__PURE__ */ h("button", {
  ...
  // src/assets/javascripts/components/content/code/index.ts
  var index = 0;
  function watchCodeBlock(el, {viewport$: viewport$2}) {
    const container$ = of(el).pipe(switchMap((child) => {
      const container = child.closest("[data-tabs]");
      if (container instanceof HTMLElement) {
        return merge(...getElements("input", container).map((input) => fromEvent(input, "change")));
      }
      return NEVER;
    }));
    return merge(viewport$2.pipe(distinctUntilKeyChanged("size")), container$).pipe(map(() => {
      const visible = getElementSize(el);
      const content = getElementContentSize(el);
      return {
  ...
  function mountCodeBlock(el, options) {
    ...
    return watchCodeBlock(el, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }

  // src/assets/javascripts/components/content/details/index.ts
  function watchDetails(el, {target$: target$2, print$: print$2}) {
    return target$2.pipe(map((target) => target.closest("details:not([open])")), filter((details) => el === details), mergeWith(print$2), mapTo(el));
  }
  function mountDetails(el, options) {
    ...
    return watchDetails(el, options).pipe(tap(internal$), finalize(() => internal$.complete()), mapTo({ref: el}));
  }

  // src/assets/javascripts/components/content/table/index.ts
  var sentinel = createElement("table");
  ...
  function mountContent(el, {target$: target$2, viewport$: viewport$2, print$: print$2}) {
    return merge(...getElements("pre > code", el).map((child) => mountCodeBlock(child, {viewport$: viewport$2})), ...getElements("table:not([class])", el).map((child) => mountDataTable(child)), ...getElements("details", el).map((child) => mountDetails(child, {target$: target$2, print$: print$2})));
  }
  ...
  function mountDialog(el, options) {
    ...
    return watchDialog(el, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function mountHeaderTitle(el, options) {
    ...
    if (typeof headline === "undefined")
      return NEVER;
    return watchHeaderTitle(headline, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function mountPalette(el) {
    ...
    return watchPalette(inputs).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function setupClipboardJS({alert$: alert$2}) {
    if (import_clipboard2.default.isSupported()) {
      new Observable((subscriber) => {
        new import_clipboard2.default("[data-clipboard-target], [data-clipboard-text]").on("success", (ev) => subscriber.next(ev));
  ...
  function setupInstantLoading({document$: document$2, location$: location$2, viewport$: viewport$2}) {
    ...
    const push$ = requestXML(`${config4.base}/sitemap.xml`).pipe(map((sitemap) => preprocess(getElements("loc", sitemap).map((node) => node.textContent))), switchMap((urls) => fromEvent(document.body, "click").pipe(filter((ev) => !ev.metaKey && !ev.ctrlKey), switchMap((ev) => {
    ...
    document$2.pipe(skip(1), map(() => getComponentElement("container")), switchMap((el) => of(...getElements("script", el))), concatMap((el) => {
      const script2 = createElement("script");
      if (el.src) {
  ...
  function setupSearchIndex({config: config4, docs, index: index2}) {
    ...
    return {config: config4, docs, index: index2, pipeline};
  }
      ...
      if (isSearchResultMessage(message)) {
        for (const result of message.data)
          for (const document2 of result)
            document2.location = `${config4.base}/${document2.location}`;
      }
      return message;
    }), share());
  ...
  function setupVersionSelector() {
    const config4 = configuration();
    requestJSON(new URL("versions.json", config4.base)).subscribe((versions) => {
  ...
  function watchSearchQuery(el) {
    ...
    const value$ = merge(fromEvent(el, "keyup"), fromEvent(el, "focus").pipe(delay(1))).pipe(map(() => fn(el.value)), distinctUntilChanged());
  ...
  function mountSearchQuery(el, {tx$}) {
    ...
    return watchSearchQuery(el).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function mountSearchResult(el, {rx$}, {query$}) {
    ...
    internal$.pipe(observeOn(animationFrameScheduler), tap(() => resetSearchResultList(list)), switchMap(({data}) => merge(of(...data.slice(0, 10)), of(...data.slice(10)).pipe(bufferCount(4), zipWith(boundary$), switchMap(([chunk]) => of(...chunk)))))).subscribe((result) => {
      addToSearchResultList(list, renderSearchResult(result));
    });
    const result$ = rx$.pipe(filter(isSearchResultMessage), map(({data}) => ({data})), startWith({data: []}));
    return result$.pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function mountSearch(el, {index$: index$2, keyboard$: keyboard$2}) {
    ...
    return merge(query$, mountSearchResult(result, worker, {query$}));
  }
  ...
  function mountSidebar(el, _a2) {
    ...
    return watchSidebar(el, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function mountSource(el) {
    ...
    return watchSource(el).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  function mountTabs(el, options) {
    ...
    return watchTabs(el, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }

  // src/assets/javascripts/components/toc/index.ts
  function watchTableOfContents(anchors, {viewport$: viewport$2, header$: header$2}) {
    const table = new Map();
    ...
    const partition$ = watchElementSize(document.body).pipe(distinctUntilKeyChanged("height"), map(() => {
      let path = [];
      return [...table].reduce((index2, [anchor, target]) => {
        while (path.length) {
    ...
    }), map((index2) => new Map([...index2].sort(([, a], [, b]) => a - b))), switchMap((index2) => combineLatest([adjust$, viewport$2]).pipe(scan(([prev, next], [adjust, {offset: {y}}]) => {
      while (next.length) {
        const [, offset] = next[0];
        if (offset - adjust < y) {
  ...
  function mountTableOfContents(el, options) {
    ...
    return watchTableOfContents(anchors, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }

  // src/assets/javascripts/components/top/index.ts
  function watchBackToTop(_el, {viewport$: viewport$2, main$: main$2}) {
    const direction$ = viewport$2.pipe(map(({offset: {y}}) => y), bufferCount(2, 1), map(([a, b]) => a > b), distinctUntilChanged());
  ...
  function mountBackToTop(el, options) {
    const internal$ = new Subject();
    internal$.pipe(observeOn(animationFrameScheduler)).subscribe({
      next({hidden}) {
        if (hidden)
    ...
    return watchBackToTop(el, options).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }
  ...
  var index$ = document.forms.namedItem("search") ? (__search == null ? void 0 : __search.index) || requestJSON(`${config3.base}/search/search_index.json`) : NEVER;
  ...
  var content$ = defer(() => merge(...getComponentElements("content").map((el) => mountContent(el, {target$, viewport$, print$})), ...getComponentElements("header-title").map((el) => mountHeaderTitle(el, {viewport$, header$})), ...getComponentElements("sidebar").map((el) => el.getAttribute("data-md-type") === "navigation" ? at(screen$, () => mountSidebar(el, {viewport$, header$, main$})) : at(tablet$, () => mountSidebar(el, {viewport$, header$, main$}))), ...getComponentElements("tabs").map((el) => mountTabs(el, {viewport$, header$})), ...getComponentElements("toc").map((el) => mountTableOfContents(el, {viewport$, header$})), ...getComponentElements("top").map((el) => mountBackToTop(el, {viewport$, main$}))));
