(() => {
  ...
  var Observable = function() {
    ...
    Observable9.prototype.lift = function(operator) {
      var observable2 = new Observable9();
    ...
    Observable9.prototype._deprecatedSyncErrorSubscribe = function(subscriber) {
    ...
    return new Observable(function(subscriber) {
      ...
      subscriber.add(scheduler.schedule(function() {
        ...
        caughtSchedule(subscriber, scheduler, function() {
          var _a = iterator2.next(), value = _a.value, done = _a.done;
          if (done) {
            subscriber.complete();
          } else {
            subscriber.next(value);
  ...
  function share(options) {
    ...
    var _a = options.connector, connector = _a === void 0 ? function() {
      return new Subject();
    } : _a, _b = options.resetOnComplete, resetOnComplete = _b === void 0 ? true : _b, _c = options.resetOnError, resetOnError = _c === void 0 ? true : _c, _d = options.resetOnRefCountZero, resetOnRefCountZero = _d === void 0 ? true : _d;
    var connection = null;
    var subject = null;
    var refCount = 0;
  ...
  function renderPublicSponsor(sponsor) {
  ...
  function mountIconSearchResult(el, {index$, query$}) {
    ...
    const boundary$ = watchElementThreshold(el).pipe(filter(Boolean));
    ...
    return watchIconSearchResult(el, {query$, index$}).pipe(tap(internal$), finalize(() => internal$.complete()), map((state) => __spreadValues({ref: el}, state)));
  }

  // src/overrides/assets/javascripts/components/iconsearch/_/index.ts
  function mountIconSearch(el) {
    const config3 = configuration();
    const index$ = requestJSON(`${config3.base}/overrides/assets/javascripts/iconsearch_index.json`);
  ...
  var SponsorType;
  (function(SponsorType2) {
    SponsorType2["PUBLIC"] = "PUBLIC";
    SponsorType2["PRIVATE"] = "PRIVATE";
  })(SponsorType || (SponsorType = {}));
  function mountSponsorship(el) {
    const sponsorship$ = requestJSON("https://gpiqp43wvb.execute-api.us-east-1.amazonaws.com/_/");
    ...
    sponsorship$.subscribe((sponsorship) => {
      ...
      list.appendChild(renderPrivateSponsor(sponsorship.sponsors.filter(({type}) => type === SponsorType.PRIVATE).length));
      count.innerText = `${sponsorship.sponsors.length}`;
      total.innerText = `$ ${sponsorship.total.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
    });
