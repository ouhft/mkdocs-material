(() => {
  ...
  function setupSearchHighlighter(config) {
    ...
    return (query) => {
      ...
      return (value) => value.replace(match, highlight).replace(/<\/mark>(\s+)<mark[^>]*>/img, "$1");
    };
  }
  ...
  var Search = class {
    constructor({config, docs, pipeline, index: index2}) {
      ...
      if (typeof index2 === "undefined") {
        ...
        this.index = lunr.Index.load(index2);
      }
    }
    search(query) {
      if (query) {
        try {
          ...
          const groups = this.index.search(`${query}*`).reduce((results, {ref, score, matchData}) => {
            ...
            return results;
          }, []).sort((a, b) => b.score - a.score).reduce((results, result) => {
